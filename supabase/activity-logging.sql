...
-- Creates auth/purchase activity tables and safer policies.
-- Anonymous browser inserts are intentionally blocked to reduce spam/cost abuse.
-- If you still want public telemetry, send it through a Supabase Edge Function instead.
...
drop policy if exists "auth activity insert authenticated" on public.auth_activity_log;
create policy "auth activity insert authenticated"
...
drop policy if exists "purchase activity insert authenticated" on public.purchase_activity_log;
create policy "purchase activity insert authenticated"
